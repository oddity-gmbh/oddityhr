...
class TestUserService(BaseTestCase):
    ...
    def test_add_user_invalid_json_keys(self):
        ...
        with self.client:
            response = self.client.post(
                '/users',
                data=json.dumps({'email': 'fred@fred.org'}),
                content_type='application/json',
            )
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 400)
    ...
    def test_single_user(self):
        ...
        user = add_user('fred', 'fred@fred.org')
        with self.client:
    ...
    def test_all_users(self):
        ...
        add_user('fred', 'fred@fred.org')
        add_user('homer', 'homer@simpson.com')
        with self.client:
            ...
            self.assertIn(
                'fred@fred.org', data['data']['users'][0]['email'])
            self.assertIn('homer', data['data']['users'][1]['username'])
            self.assertIn(
                'homer@simpson.com', data['data']['users'][1]['email'])
            self.assertIn('success', data['status'])
    ...
    def test_main_add_user(self):
        ...
        with self.client:
            response = self.client.post(
                '/',
                data=dict(username='fred', email='fred@fred.com'),
